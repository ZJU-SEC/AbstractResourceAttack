...
//#include "llvm/Transforms/PexCFI/internal.h"
//#include "utility.h"
...
class GatlinPass : public PassInfoMixin<GatlinPass> {
	public:
		GatlinPass(){}
...
ModulePass* createGatlinPass();
}
*/

//namespace{

//extern ConstInst2Func idcs2callee;

void str_truncate_dot_number(std::string&);
StructType* find_assignment_to_struct_type(Value*, const Indices &, ValueSet&);
...
class GatlinModule : public ModulePass
{
    public:
	   ...
	   void process_cpgf();
        ...
        void collect_kernel_init_functions();
        void collect_wrappers();
        void collect_crits();
        void collect_chkps();
        void identify_interesting_struct();
        void identify_kmi();
        void identify_dynamic_kmi();

        void populate_indcall_list_using_cvf();
        void populate_indcall_list_through_kmi();
         ...
         * analyze
         */
        void check_critical_function_usage();
        void check_critical_variable_usage();
        void check_critical_type_field_usage();
        /*
         * thread worker
         */
        ...
        void dump_kmi();
        void dump_dkmi();
        void handle_structs_in_cmp(CmpInst* cmp);
        void populate_between_diff_structs(StoreInst* i);
        void use_of_structs(Value *v, std::unordered_set<struct_field >& si);
    ...
    public:
//private objects
        std::unordered_set<struct_field> source, destination;
...
// extern std::unordered_map<std::unordered_set<struct_field> *, std::unordered_set<Value *, std::unordered_set<Function *> > > set2fu;

//stores all indirect call sites
extern InDirectCallSites idcs;
extern std::unordered_map<std::unordered_set<struct_field> *, std::unordered_set<CallInst *> > set2ci;
extern std::unordered_map<struct_field, std::unordered_set<struct_field> *> si_disjoint_sets;
extern std::unordered_map<Function *, std::unordered_map<Value *, std::unordered_set<struct_field> > > fu2si;
extern std::unordered_map<CallInst *, std::unordered_set<struct_field> > ci2si;
extern std::unordered_map<std::unordered_set<struct_field> *, std::unordered_map<Value *, std::unordered_set<Function *> > > set2fu;
